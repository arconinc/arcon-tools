...
export default function VendorDetailPage() {
  ...
  useEffect(() => {
    if (vendor) {
      setTagIds((vendor.tags ?? []).map((t) => t.id))
      if (vendor.billing_state && !taxState) setTaxState(vendor.billing_state)
    }
  ...
  async function handleTagsChange(newIds: string[]) {
    ...
    setTagSaving(true)
    try {
      await fetch(`/api/marketing/vendors/${vendor.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  useEffect(() => {
    if (isNew) return
    fetch(`/api/marketing/vendors/${id}`)
      .then((r) => r.json())
  ...
  async function saveEdit() {
    ...
    setSaving(true)
    try {
      const res = await fetch(`/api/marketing/vendors/${vendor.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
          ...
          <div className="space-y-3">
            ...
            <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
              <div className="px-4 py-3 border-b border-slate-100 bg-slate-50 flex items-center gap-2">
                ...
                <h2 className="text-sm font-semibold text-slate-700">Tax Exempt Forms</h2>
              </div>
              <div className="p-4 space-y-3">
                ...
                {taxState && (() => {
                  const recommended = recommendTaxForms(taxState, taxForms)
                  if (recommended.length === 0) return (
                    <p className="text-xs text-slate-400 italic">No vendor forms found for {taxState}.</p>
                  )
                  return (
                    <div className="space-y-2">
                      {recommended.map((form, i) => (
                        <div key={form.id} className="rounded-lg border border-slate-200 p-2.5">
                          <div className="flex items-start gap-2">
                            <div className="flex-1 min-w-0">
                              ...
                              {i > 0 && (
                                <p className="text-xs text-slate-400 mt-0.5">Alternative</p>
                              )}
                            </div>
                          </div>
                          <a
