...
export default function OpportunityDetailPage() {
  ...
  async function handleTagsChange(newIds: string[]) {
    ...
    try {
      await fetch(`/api/marketing/opportunities/${opp.id}`, {
        method: 'PATCH',
  ...
  async function saveEdit() {
    ...
    try {
      ...
      const res = await fetch(`/api/marketing/opportunities/${opp.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  async function handleStageClick(stage: PipelineStage) {
    ...
    try {
      ...
      const full = await fetch(`/api/marketing/opportunities/${opp.id}`).then((r) => r.json())
          ...
          <div className="flex items-center justify-between px-5 py-3 border-b border-slate-100 bg-slate-50">
            ...
            {!editing ? (
              <button onClick={startEdit}
                className="px-3 py-1 text-xs font-semibold border border-slate-300 text-slate-600 rounded-lg hover:bg-white transition-colors">
                Edit
              </button>
            ) : (
              ...
              <>
                ...
                <div>
                  ...
                  <div className="px-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 text-slate-700">
                    {opp.customer?.name ?? opp.customer_id}
                  </div>
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1">Pipeline Stage</label>
