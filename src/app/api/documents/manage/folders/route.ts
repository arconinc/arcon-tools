...
export async function POST(request: Request) {
  const { section_id, name, sort_order, required_role } = await request.json()
  ...
  const { data, error } = await adminClient
    .from('doc_folders')
    .insert({ section_id, name: name.trim(), sort_order: sort_order ?? 0, required_role: required_role ?? null })
    .select()
    .single()
