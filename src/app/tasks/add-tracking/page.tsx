...
interface OrderSearchResult extends PromoOrder {}
...
export default function AddTrackingPage() {
  ...
  function handleOrderSelected(order: OrderSearchResult) {
  ...
  function handleSuccess(data: SuccessData) {
    setSuccessData(data)
    setStep(data.partial ? 'partial' : 'success')
  }
    ...
    return (
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 text-center">
          ...
          <h3 className="font-semibold text-amber-800 mb-1">No Store Selected</h3>
          <p className="text-sm text-amber-700">Please select a store from the header dropdown before searching for orders.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl mx-auto">
      ...
      {step === 'search' && (
        <OrderSearch storeId={selectedStore.store_id} onSelect={handleOrderSelected} />
      )}
...
function PageHeader() {
  return (
    <div className="mb-6">
      <h1 className="text-2xl font-bold text-slate-900">Add Tracking Number</h1>
      <p className="text-sm text-slate-500 mt-1">
        Search for an order, enter a tracking number, and send a shipment notification to the customer.
      </p>
    </div>
  )
}
...
function OrderSearch({ storeId, onSelect }: { storeId: string; onSelect: (o: OrderSearchResult) => void }) {
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{formatDate(order.created)} · ${order.amount}</p>
                </div>
...
function TrackingForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.status === 207) {
      // Partial — shipment added but email failed
      onSuccess({
        orderId: order.id,
        customerEmail: detail?.billingEmail ?? '',
        trackingNumber,
    ...
    if (!res.ok) {
      setSubmitError(data.error ?? 'Failed to submit tracking')
      return
    }
        ...
        {detail && (
          <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-sm mb-4">
            <div>
              ...
              <p className="font-medium text-slate-800">{detail.firstName} {detail.lastName}</p>
            </div>
            <div>
              <span className="text-slate-500">Email</span>
              <p className="font-medium text-slate-800">{detail.billingEmail}</p>
            </div>
            <div>
              <span className="text-slate-500">Status</span>
              <p>
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusColor(detail.status)}`}>
                  {detail.status}
                </span>
              </p>
            </div>
            <div>
              ...
              <p className="font-medium text-slate-800">${detail.amount}</p>
            </div>
          </div>
        )}
              ...
              {products.map((p) => (
                <div key={p.id} className="flex justify-between text-sm py-1.5 border-t border-slate-100">
                  <div>
                    ...
                    <span className="text-slate-400 ml-2 text-xs">SKU: {p.sku}</span>
                  </div>
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6">
        ...
        <div className="mb-5 p-3.5 bg-amber-50 border border-amber-200 rounded-xl flex gap-3">
          ...
          <p className="text-sm text-amber-800">
            <strong>Do not submit more than once.</strong> Each submission creates a new shipment record on this order.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <span className="text-sm text-slate-700">
              I confirm this shipment has not already been submitted for order <strong>#{order.id}</strong>.
            </span>
          </label>

          <button
            type="submit"
            disabled={submitting || !trackingNumber || !confirmed}
    ...
    return (
      <div className="bg-white border border-amber-200 rounded-2xl p-8 text-center">
        ...
        <p className="text-slate-600 text-sm mb-1">
          Tracking number <strong>{data.trackingNumber}</strong> ({data.carrier}) was added to order <strong>#{data.orderId}</strong>.
        </p>
        <p className="text-sm text-red-600 mb-4">
          However, the notification email to <strong>{data.customerEmail}</strong> could not be sent.
          <br />
          <span className="text-xs text-slate-400">{data.emailError}</span>
        </p>
        <p className="text-sm text-slate-500 mb-6">
          You may manually send the notification from the PromoBullit dashboard, or retry this order.
        </p>
        <button
          onClick={onReset}
          className="px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-medium transition-colors"
        >
          Add Tracking to Another Order
        </button>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white border border-emerald-200 rounded-2xl p-8 text-center">
      ...
      <h2 className="text-xl font-bold text-slate-900 mb-2">
        {data.emailSent ? 'Tracking Added & Email Sent!' : 'Tracking Added!'}
      </h2>
      <p className="text-slate-600 text-sm mb-1">
        Tracking number <strong className="font-mono">{data.trackingNumber}</strong> ({data.carrier}) has been added to order <strong>#{data.orderId}</strong>.
      </p>
      <p className="text-sm text-slate-500 mb-6">
        {data.emailSent
          ? <>A shipment notification email was sent to <strong>{data.customerEmail}</strong>.</>
